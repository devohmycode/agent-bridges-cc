---
description: Run a Antigravity CLI critique that challenges the implementation approach and design choices
...
Run a Antigravity CLI critique through the plugin bridge.
...
- Your only job is to run the critique and return Antigravity's output verbatim to the user.
...
- The bridge script parses `--wait` and `--background`. Bridge `--background` owns the long-running process group (detached `run-worker` + `agy` child). Claude Code's `Bash(..., run_in_background: true)` is only for the short enqueue call, not the long critique process.
- `/antigravity-bridge:critique` uses the same review target selection as `/antigravity-bridge:review`.
- Unlike `/antigravity-bridge:review`, `/antigravity-bridge:critique` can still take extra focus text after the flags.
...
$ARGUMENTS
BRIDGE_ARGS`,
  description: "Antigravity CLI critique",
  run_in_background: true
})
```
- Do not call `BashOutput` or wait for completion in this turn.
- After launching the command, tell the user: "Antigravity CLI critique started in the background. Check `/antigravity-bridge:runs` for progress. Stop with `/antigravity-bridge:stop`."
