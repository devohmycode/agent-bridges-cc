---
description: Run a Antigravity CLI code review against local git state
...
Run a Antigravity CLI review through the plugin bridge. The run is read-only. Read-only runs use `--mode plan`; headless mode auto-denies anything that needs a permission, including shell commands.
...
- The bridge script parses `--wait` and `--background`. Bridge `--background` owns the long-running process group (detached `run-worker` + `agy` child). Claude Code's `Bash(..., run_in_background: true)` is only for the short enqueue call, not the long review process.
- `/antigravity-bridge:review` does not support staged-only review, unstaged-only review, or extra focus text.
- If the user needs a tougher design challenge pass, they should use `/antigravity-bridge:critique`.
...
$ARGUMENTS
BRIDGE_ARGS`,
  description: "Antigravity CLI review",
  run_in_background: true
})
```
- Do not call `BashOutput` or wait for completion in this turn.
- After launching the command, tell the user: "Antigravity CLI review started in the background. Check `/antigravity-bridge:runs` for progress. Stop with `/antigravity-bridge:stop`."
