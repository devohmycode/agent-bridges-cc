---
description: Run a GitHub Copilot CLI critique that challenges the implementation approach and design choices
...
Run a GitHub Copilot CLI critique through the plugin bridge.
...
- Your only job is to run the critique and return Copilot's output verbatim to the user.
...
- Leave `--model` and `--effort` unset unless the user explicitly asks for them. Accepted effort values: `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`.
- The bridge script parses `--wait` and `--background`. Bridge `--background` owns the long-running process group (detached `run-worker` + `copilot` child). Claude Code's `Bash(..., run_in_background: true)` is only for the short enqueue call, not the long critique process.
- `/copilot-bridge:critique` uses the same review target selection as `/copilot-bridge:review`.
- Unlike `/copilot-bridge:review`, `/copilot-bridge:critique` can still take extra focus text after the flags.
...
- Run:
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/bridge.mjs" critique --args-stdin <<'BRIDGE_ARGS'
$ARGUMENTS
BRIDGE_ARGS
```
...
```typescript
Bash({
  command: `node "${CLAUDE_PLUGIN_ROOT}/scripts/bridge.mjs" critique --background --args-stdin <<'BRIDGE_ARGS'
$ARGUMENTS
BRIDGE_ARGS`,
  description: "GitHub Copilot CLI critique",
  run_in_background: true
})
```
- Do not call `BashOutput` or wait for completion in this turn.
- After launching the command, tell the user: "GitHub Copilot CLI critique started in the background. Check `/copilot-bridge:runs` for progress. Stop with `/copilot-bridge:stop`."
