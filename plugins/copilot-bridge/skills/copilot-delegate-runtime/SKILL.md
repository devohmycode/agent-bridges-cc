...
- Do not call `check`, `review`, `critique`, `runs`, `show`, or `stop` from `copilot-bridge:copilot-delegate`.
...
- Accepted effort values: `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`.
- `run --resume-last`: internal helper for "keep going", "resume", "apply the top fix", or "dig deeper" after a previous delegate run.

Safety rules:
- Default to write-capable Copilot work in `copilot-bridge:copilot-delegate` unless the user explicitly asks for read-only behavior.
