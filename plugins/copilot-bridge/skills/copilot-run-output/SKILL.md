---
name: copilot-run-output
description: Internal guidance for presenting GitHub Copilot CLI bridge output back to the user
user-invocable: false
---

# GitHub Copilot CLI Run Output
...
- For `copilot-bridge:copilot-delegate`, do not turn a failed or incomplete Copilot run into a Claude-side implementation attempt. Report the failure and stop.
- For `copilot-bridge:copilot-delegate`, if Copilot was never successfully invoked, do not generate a substitute answer at all.
...
- If the helper reports that setup or authentication is required, direct the user to `/copilot-bridge:check` and do not improvise alternate auth flows.
