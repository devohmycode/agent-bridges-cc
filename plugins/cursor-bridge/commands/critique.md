---
description: Run a Cursor Agent critique that challenges the implementation approach and design choices
argument-hint: '[--wait|--background] [--base <ref>] [--scope auto|working-tree|branch] [--model <model>] [focus ...]'
disable-model-invocation: true
allowed-tools: Read, Glob, Grep, Bash(node:*), Bash(git:*), AskUserQuestion
---

Run a Cursor Agent critique through the plugin bridge.
...
- Leave `--model` and `--effort` unset unless the user explicitly asks for them. `cursor-agent` has no reasoning-effort setting; the bridge ignores `--effort`.
...
- `/cursor-bridge:critique` uses the same review target selection as `/cursor-bridge:review`.
- Unlike `/cursor-bridge:review`, `/cursor-bridge:critique` can still take extra focus text after the flags.
...
$ARGUMENTS
BRIDGE_ARGS`,
  description: "Cursor Agent critique",
  run_in_background: true
})
```
- Do not call `BashOutput` or wait for completion in this turn.
- After launching the command, tell the user: "Cursor Agent critique started in the background. Check `/cursor-bridge:runs` for progress. Stop with `/cursor-bridge:stop`."
