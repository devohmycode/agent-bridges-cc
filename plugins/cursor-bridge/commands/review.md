...
Run a Cursor Agent review through the plugin bridge. The run is read-only. Read-only runs use `--mode ask` (read-only Q&A).
...
- `/cursor-bridge:review` does not support staged-only review, unstaged-only review, or extra focus text.
- If the user needs a tougher design challenge pass, they should use `/cursor-bridge:critique`.
...
- After launching the command, tell the user: "Cursor Agent review started in the background. Check `/cursor-bridge:runs` for progress. Stop with `/cursor-bridge:stop`."
