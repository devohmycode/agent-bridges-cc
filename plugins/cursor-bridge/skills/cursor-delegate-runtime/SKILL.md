...
# Cursor Agent Delegate Runtime

Use this skill only inside the `cursor-bridge:cursor-delegate` subagent.
...
- Do not call `check`, `review`, `critique`, `runs`, `show`, or `stop` from `cursor-bridge:cursor-delegate`.
...
- `cursor-agent` has no reasoning-effort setting; the bridge ignores `--effort`.
- `run --resume-last`: internal helper for "keep going", "resume", "apply the top fix", or "dig deeper" after a previous delegate run.

Safety rules:
- Default to write-capable Cursor work in `cursor-bridge:cursor-delegate` unless the user explicitly asks for read-only behavior.
