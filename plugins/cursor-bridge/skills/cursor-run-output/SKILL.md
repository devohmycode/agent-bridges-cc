...
description: Internal guidance for presenting Cursor Agent bridge output back to the user
user-invocable: false
---

# Cursor Agent Run Output
...
- For `cursor-bridge:cursor-delegate`, do not turn a failed or incomplete Cursor run into a Claude-side implementation attempt. Report the failure and stop.
- For `cursor-bridge:cursor-delegate`, if Cursor was never successfully invoked, do not generate a substitute answer at all.
...
- If the helper reports that setup or authentication is required, direct the user to `/cursor-bridge:check` and do not improvise alternate auth flows.
