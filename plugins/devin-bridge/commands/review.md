---
description: Run a Devin CLI code review against local git state
argument-hint: '[--wait|--background] [--base <ref>] [--scope auto|working-tree|branch] [--model <model>]'
disable-model-invocation: true
allowed-tools: Read, Glob, Grep, Bash(node:*), Bash(git:*), AskUserQuestion
---

Run a Devin CLI review through the plugin bridge. The run is read-only. Read-only runs use `--permission-mode auto`, which auto-approves read-only tools and rejects the rest.
...
- The bridge script parses `--wait` and `--background`. Bridge `--background` owns the long-running process group (detached `run-worker` + `devin` child). Claude Code's `Bash(..., run_in_background: true)` is only for the short enqueue call, not the long review process.
...
- After launching the command, tell the user: "Devin CLI review started in the background. Check `/devin-bridge:runs` for progress. Stop with `/devin-bridge:stop`."
