---
name: devin-delegate-runtime
description: Internal helper contract for calling the bridge runtime from Claude Code
user-invocable: false
---

# Devin CLI Delegate Runtime

Use this skill only inside the `devin-bridge:devin-delegate` subagent.
...
- Do not call `check`, `review`, `critique`, `runs`, `show`, or `stop` from `devin-bridge:devin-delegate`.
...
- `devin` has no reasoning-effort setting; the bridge ignores `--effort`.
- `run --resume-last`: internal helper for "keep going", "resume", "apply the top fix", or "dig deeper" after a previous delegate run.

Safety rules:
- Default to write-capable Devin work in `devin-bridge:devin-delegate` unless the user explicitly asks for read-only behavior.
