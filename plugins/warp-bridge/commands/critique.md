---
description: Run a Warp Oz critique that challenges the implementation approach and design choices
argument-hint: '[--wait|--background] [--base <ref>] [--scope auto|working-tree|branch] [--model <model>] [focus ...]'
disable-model-invocation: true
allowed-tools: Read, Glob, Grep, Bash(node:*), Bash(git:*), AskUserQuestion
---

Run a Warp Oz critique through the plugin bridge.
...
- Your only job is to run the critique and return Warp's output verbatim to the user.
...
- Leave `--model` and `--effort` unset unless the user explicitly asks for them. `oz` has no reasoning-effort setting; the bridge ignores `--effort`.
- The bridge script parses `--wait` and `--background`. Bridge `--background` owns the long-running process group (detached `run-worker` + `oz` child). Claude Code's `Bash(..., run_in_background: true)` is only for the short enqueue call, not the long critique process.
- `/warp-bridge:critique` uses the same review target selection as `/warp-bridge:review`.
- Unlike `/warp-bridge:review`, `/warp-bridge:critique` can still take extra focus text after the flags.
...
$ARGUMENTS
BRIDGE_ARGS`,
  description: "Warp Oz critique",
  run_in_background: true
})
```
- Do not call `BashOutput` or wait for completion in this turn.
- After launching the command, tell the user: "Warp Oz critique started in the background. Check `/warp-bridge:runs` for progress. Stop with `/warp-bridge:stop`."
