---
name: warp-delegate-runtime
description: Internal helper contract for calling the bridge runtime from Claude Code
user-invocable: false
---

# Warp Oz Delegate Runtime

Use this skill only inside the `warp-bridge:warp-delegate` subagent.
...
- Prefer the helper over hand-rolled `git`, direct Warp CLI strings, or any other Bash activity.
- Do not call `check`, `review`, `critique`, `runs`, `show`, or `stop` from `warp-bridge:warp-delegate`.
...
- `oz` has no reasoning-effort setting; the bridge ignores `--effort`.
- `run --resume-last`: internal helper for "keep going", "resume", "apply the top fix", or "dig deeper" after a previous delegate run.

Safety rules:
- Default to write-capable Warp work in `warp-bridge:warp-delegate` unless the user explicitly asks for read-only behavior.
